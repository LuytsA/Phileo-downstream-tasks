...
def unpatchify(n, c, h, w, n_patches, tensors):
    ...
    assert h == w, "Unpatchify method is implemented for square images only"
...
class MyViT_decoder(nn.Module):
  ...
  def forward(self, x):
      ...
      return out
class ViT(nn.Module):
    def __init__(self, chw=(10, 64, 64), n_patches=4, n_blocks=2, hidden_d=768, n_heads=12):
        # Super constructor
...
class autoencoderViT(nn.Module):
    ...
    def __init__(self, chw=(10, 128, 128), n_patches=8, n_blocks=2, hidden_d=768, n_heads=12,
                 decoder_n_blocks=2, decoder_hidden_d=512, decoder_n_heads=16):
        # Super constructor
